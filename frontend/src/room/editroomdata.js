...
const EditroomInfo = () => {
    ...
    console.log(id);
    useEffect(() => {
        axios.get('http://20.115.68.51:5000/api/getsingle-user/' + id).then((res) => { console.log(res.data.response)
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        axios.put(`http://20.115.68.51:1000/api/updateuser/${id}`, formdata)
            .then((res) => { 
                alert("Updated Sucessfully"); 
                console.log(res.data);
                window.location.replace(
                   "http://20.115.68.51:3001/room/getroomdata"
                  );
            })
            .catch((error) => {
