...
const Editdata = () =>{
    ...
    console.log(id);
    useEffect(()=>{
        axios.get('http://20.115.68.51:5000/api/get-single-dth/'+id).then((res)=>{console.log(res.data.response)
    ...
    const handlesubmit=(e)=>{
        e.preventDefault();
        axios.put(`http://20.115.68.51:5000/api/update-dth/${id}`, formdata).then((res)=>{
            alert("Updated Sucessfully"); 
                console.log(res.data);
                window.location.replace(
                   "http://20.115.68.51:3000/DThpage/getdata"
                  );
        })
    }
    return(
